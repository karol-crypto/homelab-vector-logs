...
The logs are received by python script _log_script_receiver_. For the script to work we should allow http port in Windows Firewall. To do this we can use this command in PowerShell _New-NetFirewallRule -DisplayName "Python HTTP Log Receiver" -Direction Inbound -Protocol TCP -LocalPort 8080 -Action Allow_
